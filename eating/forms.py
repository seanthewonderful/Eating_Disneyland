...
class RegisterForm(FlaskForm):
    username = StringField(
        "Username",
        render_kw={"placeholder": "Unique Username"},
    ...
    email = EmailField(
        "Email", render_kw={"placeholder": "Email Address"}, validators=[Email()]
    )
    age = IntegerField(
        "Age",
        render_kw={"placeholder": "Your age"},
        validators=[NumberRange(min=8, max=120)],
    )
    zipcode = StringField(
        "Zip Code",
        render_kw={"placeholder": "Your Zip Code"},
        validators=[
            Length(min=5, max=5, message="Only U.S. zipcodes accepted(5 numbers total)")
        ],
    )
