...
from eating.crud import (
    total_ratings,
    star_avg,
    total_ratings_fountain,
    star_avg_fountain,
)


def make_map(restaurants):
    disneyland_map = folium.Map(
    ...
    for each in restaurants:
        ...
        popup = folium.Popup(
            html.format(
                ...
                each.expense,
                each.land,
                url_for("restaurants_bp.eating_place", rest_id=each.rest_id),
                star_avg(each.rest_id, total_ratings(each.rest_id)),
                total_ratings(each.rest_id),
            ),
            max_width=200,
        )
        ...
        folium.Marker(
            ...
            tooltip=each.name,
            popup=popup,
            icon=folium.Icon(icon_color="white", icon="fa-cutlery", prefix="fa"),
        ).add_to(disneyland_map)

        disneyland_map.save("eating/templates/disneyland_map.html")
    ...
    for each in fountains:
        ...
        popup = folium.Popup(
            html.format(
                ...
                url_for("fountains_bp.fountain_place", fountain_id=each.id),
            ),
            max_width=200,
        )
        ...
        fountain_map.save("eating/templates/fountain_map.html")
